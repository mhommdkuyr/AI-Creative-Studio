    ...
    properties: {
      ...
      operations: {
        ...
        items: {
          type: 'object',
          required: ['op'],
          properties: geminiOperationProperties,
        },
      },
    },
  },
...
async function planWithGemini(text: string, timeline: any): Promise<EditPlan | null> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[gemini] HTTP ${response.status}: ${body.slice(0, 700)}`);
      return null;
    }
    ...
    if (!call?.functionCall?.args) {
      console.error('[gemini] no function call returned');
      return null;
    }
